...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val leftMost = try { left.mostRetweeted } catch { case _: java.util.NoSuchElementException => elem }
    val rightMost = try { right.mostRetweeted } catch { case _: java.util.NoSuchElementException => elem }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    allTweets.filter(tw => google.exists(word => tw.text.contains(word)))
  }

  lazy val appleTweets: TweetSet = {
    allTweets.filter(tw => apple.exists(word => tw.text.contains(word)))
  }
