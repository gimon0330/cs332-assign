...
object Huffman {
   ...
   *   }
   */
  def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy(c => c).map{ case (c, cs) => (c, cs.length) }.toList
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    tree match {
      case Leaf(c, _) => return if (bits.isEmpty) List(c) else List.fill(bits.length)(c)
      case _ => ()
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeAcc(t: CodeTree, c: Char, acc: List[Bit]): List[Bit] = t match {
      ...
      case Fork(l, r, _, _) => 
        ...
        else encodeAcc(r, c, acc :+ 1)
    }
    text.flatMap(c => encodeAcc(tree, c, List()))
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertAcc(t: CodeTree, path: List[Bit]): CodeTable = t match {
